 ...
 */
public class ClaimsReportsAction extends TTKAction {
	
	private static final Logger log = Logger.getLogger( ClaimsReportsAction.class );
    ...
    public ActionForward doChangeLabel(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    							       HttpServletResponse response) throws TTKException{
    	try{
    		log.debug("Inside the doChangeLabel method of ClaimsReportsAction");
    		setLinks(request);
    		...
    		log.debug("status value is :"+strStatus);
    		return this.getForward(strMISreports,mapping,request);
    	}//end of try
    	catch(TTKException expTTK)
    	{
    ...
    public ActionForward doGenerateReport(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    	...
    	try{
    		...
    		reportDetailVO.setsDomiciOption(request.getParameter("domicioption"));
    		reportDetailVO.settInwardRegister(request.getParameter("inwardregister"));
            //call the DAO to save the Report parameter details
    		int iCount=reportDetailObject.saveReportDetails(reportDetailVO); 
    		if(iCount>=0)
			{
    			log.info("Inside the doGenerateReport method of ClaimsReportsAction in iCount");
    		...
    		try
    		{
    			...
    			hashMap.put("domicioption",request.getParameter("domicioption"));
    			hashMap.put("sGroupPolicyNo",request.getParameter("sGroupPolicyNo"));
    			hashMap.put("inwardregister",request.getParameter("inwardregister"));
