...
public class ClaimBatchAction extends TTKAction
{
	private static Logger log = Logger.getLogger(ClaimBatchAction.class);
	...
	private static final  String strClaimsBatchlist="claimbatchlist";
    ...
    public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    	...
    	try{
    		log.debug("Inside the doDefault method of ClaimBatchAction");
    		setLinks(request);    		
    		
    		HttpSession session=request.getSession();
    		DynaActionForm frmClaimBatchList = (DynaActionForm)form;
			...
			if("Y".equals(request.getParameter("Entry"))){
				frmClaimBatchList.initialize(mapping);//reset the form data
			}//end of if(TTKCommon.checkNull(request.getParameter("Entry")).equals("Y"))
			
			tableData.createTableInfo("ClaimBatchListTable",new ArrayList());
			session.setAttribute("tableData",tableData);
			frmClaimBatchList.initialize(mapping);//reset the form data
			return this.getForward(strClaimsBatchlist, mapping, request);
    	}//end of try
    ...
    public ActionForward doSearch(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    												HttpServletResponse response) throws Exception{
    	try{
    		log.debug("Inside the doSearch method of ClaimBatchAction");
    		setLinks(request);
    		HttpSession session=request.getSession();
			...
			if(!strPageID.equals("") || !strSortID.equals(""))
			{
				if(!strPageID.equals(""))
				{
					...
					return mapping.findForward(strClaimsBatchlist);
			...
			else
			{
				...
				tableData.createTableInfo("ClaimBatchListTable",null);
				tableData.setSearchData(this.populateSearchCriteria((DynaActionForm)form,request));
                tableData.modifySearchData("search");
			}//end of else
			ArrayList alClaimsBatchList= claimBatchManagerObject.getClaimBatchList(tableData.getSearchData());
	...
	public ActionForward doCopyToWebBoard(ActionMapping mapping,ActionForm form,HttpServletRequest request,
																HttpServletResponse response) throws Exception{
		try{
			log.debug("Inside ClaimBatchAction doCopyToWebBoard");
			setLinks(request);
	...
	private void populateWebBoard(HttpServletRequest request)throws TTKException
	{
		...
		if(strChk!=null&&strChk.length!=0)
		{
			for(int i=0; i<strChk.length;i++)
			{
				...
				batchVO=(BatchVO)tableData.getData().get(Integer.parseInt(strChk[i]));
				cacheObject.setCacheId(this.prepareWebBoardId(batchVO));
				cacheObject.setCacheDesc(batchVO.getBatchNO());
				alCacheObject.add(cacheObject);
    ...
    public ActionForward doBackward(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    	...
    	try{
			...
			return this.getForward(strClaimsBatchlist, mapping, request);   //finally return to the grid screen
    	}//end of try
    	catch(TTKException expTTK)
		{
			return this.processExceptions(request, mapping, expTTK);
    ...
    public ActionForward doForward(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    	...
    	try{
			...
			ArrayList alClaimsBatchList = claimBenefitManagerObject.getClaimBatchList(tableData.getSearchData());
     ...
     */
    private ArrayList<Object> populateSearchCriteria(DynaActionForm frmClaimBatchList,HttpServletRequest request)
    {
    	//build the column names along with their values to be searched
    	ArrayList<Object> alSearchParams = new ArrayList<Object>();
    	alSearchParams.add(frmClaimBatchList.getString("sBatchNumber"));
    	alSearchParams.add(frmClaimBatchList.getString("sProviderName"));
    	alSearchParams.add(TTKCommon.checkNull(frmClaimBatchList.getString("sBatchReceivedFrom")));
    	alSearchParams.add(TTKCommon.checkNull(frmClaimBatchList.getString("sBatchReceivedTo")));
    	alSearchParams.add(frmClaimBatchList.getString("sBatchStatusID"));
    	alSearchParams.add(frmClaimBatchList.getString("sClaimType"));
    	alSearchParams.add(frmClaimBatchList.getString("sModeOfClaim"));
		alSearchParams.add(frmClaimBatchList.getString("sSubmissionType"));
		
    	alSearchParams.add(TTKCommon.getUserSeqId(request));
    	alSearchParams.add(frmClaimBatchList.getString("sProcessType"));
