...
function edit(rownum){
    ...
    document.forms[1].action = "/PartnerPreAuthAction.do";
    document.forms[1].submit();
}//end of edit(rownum)
...
function edit2(rownum){
    ...
    $(document).ready(function () {
		$.ajax({
            url :"/asynchronAction.do?mode=getPreAuthRefStatus&rownum="+rownum,
            dataType:"text",
			type:"POST",
			async:false,
		    context:document.body,
            success : function(responseData) {            	
            	var refStatus=responseData;
            	if(refStatus==='In-Progress'){
            				confirm("Pre-Approval is not redirected to New PA screen");
            	}else{
            	    document.forms[1].mode.value="doViewPreauth";
            		document.forms[1].leftlink.value ="Pre-Authorization";
            		document.forms[1].sublink.value ="Processing";
            		document.forms[1].tab.value ="System Preauth Approval";
            	    document.forms[1].action = "/PreAuthAction.do?newPreAuth=true";
            	    document.forms[1].submit();
            	}
              }//success data
		 });//$.ajax(	
