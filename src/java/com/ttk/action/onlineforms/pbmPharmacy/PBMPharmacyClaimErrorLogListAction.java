...
public class PBMPharmacyClaimErrorLogListAction extends TTKAction{
...
//	private static final String strClaimBatchDetails="ClaimsBatchDetails";
	...
	public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try {
			log.debug("Inside the doDefault method of ClaimLogListAction");
			setLinks(request);

			HttpSession session = request.getSession();
			DynaActionForm frmErrorLogList = (DynaActionForm) form;
			// get the tbale data from session if exists
			TableData tableData = (session.getAttribute("tableData") == null) ? new TableData()
					: (TableData) session.getAttribute("tableData");
			...
			tableData.createTableInfo("PBMClaimUploadErrorLogList",
					new ArrayList());
 ...
 HttpServletResponse response)
			throws Exception {
		try {
			log.debug("Inside the doSearch method of ClaimLogListAction");
			setLinks(request);
			HttpSession session = request.getSession();
			ClaimBatchManager claimBatchManagerObject = this
					.getClaimBatchManagerObject();
			// get the tbale data from session if exists
			TableData tableData = session.getAttribute("tableData") == null ? new TableData()
					: (TableData) session.getAttribute("tableData");
			...
			if (!strPageID.equals("") || !strSortID.equals("")) {
				...
				tableData.createTableInfo("PBMClaimUploadErrorLogList", null);
				tableData.setSearchData(this.populateSearchCriteria(
						(DynaActionForm) form, request));
				tableData.modifySearchData("search");
				tableData.getSearchData().set(
						tableData.getSearchData().size() - 3, "DESC");
			}// end of else
			ArrayList alClaimsBatchList = claimBatchManagerObject
					.getClaimUploadErrorLogList(tableData.getSearchData());
	...
	private ClaimBatchManager getClaimBatchManagerObject() throws TTKException
	{
		...
		{
			throw new TTKException(exp, strErrorLogList);
		}//end of catch
	...
	public ActionForward doBackward(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			ArrayList alClaimsBatchList = claimBatchManagerObject
					.getClaimUploadErrorLogList(tableData.getSearchData());
			tableData.setData(alClaimsBatchList, strBackward);// set the table
																// data
			request.getSession().setAttribute("tableData", tableData); // set
																		...
																		// to
																		// session
			return this.getForward(strErrorLogList, mapping, request); // finally
																		// return
																		// to
																		// the
																		// grid
	...
	public ActionForward doForward(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			setLinks(request);
			ClaimBatchManager claimBenefitManagerObject = this
					.getClaimBatchManagerObject();
			// get the tbale data from session if exists
			TableData tableData = TTKCommon.getTableData(request);
			tableData.modifySearchData(strForward);// modify the search data
			ArrayList alClaimsBatchList = claimBenefitManagerObject
					.getClaimUploadErrorLogList(tableData.getSearchData());
			tableData.setData(alClaimsBatchList, strForward);// set the table
																// data
			request.getSession().setAttribute("tableData", tableData); // set
																		// the
																		// table
	...
	public ActionForward doViewErrorLog(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		try {
			...
			if (!(TTKCommon.checkNull(request.getParameter(strRownum))
					.equals(""))) {
				claimUploadErrorLogVO = (ClaimUploadErrorLogVO) tableData
						.getRowInfo(Integer.parseInt((String) request
								.getParameter(strRownum)));
				// claimUploadErrorLog
				...
				try {
					emptyReport = JasperCompileManager
							.compileReport("generalreports/EmptyReprot.jrxml");
					HashMap<String, String> hashMap = new HashMap<String, String>();
					ByteArrayOutputStream boas = new ByteArrayOutputStream();
					mainTtkReportDataSource = new TTKReportDataSource(
							strReportId, parameter);
					ResultSet main_report_RS = mainTtkReportDataSource
							.getResultData();
					...
					if (main_report_RS != null && main_report_RS.next()) {
						...
						mainJasperPrint = JasperFillManager.fillReport(
								mainJasperReport, hashMap,
								mainTtkReportDataSource);

					} else {

						mainJasperPrint = JasperFillManager.fillReport(
								emptyReport, hashMap, new JREmptyDataSource());
					}

					if ("EXCEL".equals(request.getParameter("reportType"))) {

						JRXlsExporter jExcelApiExporter = new JRXlsExporter();
						jExcelApiExporter.setParameter(
								JRXlsExporterParameter.JASPER_PRINT,
								mainJasperPrint);
						jExcelApiExporter.setParameter(
								JRXlsExporterParameter.OUTPUT_STREAM, boas);
						jExcelApiExporter.setParameter(
					...
					return this.processExceptions(request, mapping,
							new TTKException(exp, strClaimBatchSearchError));
				}
