...
/**
 * @ (#) DHPOGlobalNetNewTransaction.java NOV 11 2016
 * Project      : Project X
...
public class DHPOGlobalNetClaimDownload implements Job {
	
	private static final String strDHPOGlobalNetClaimDownloadError="DHPOGlobalNetClaimDownload";
	private static final Logger log = Logger.getLogger( DHPOGlobalNetClaimDownload.class );
	...
	public void execute(JobExecutionContext arg0) {	
		...
		try
		{	
			if(new Boolean(TTKPropertiesReader.getPropertyValue("DHPO.GNCD.WS.RUN.MODE"))){
			
			log.info("DHPO GlobalNet Claim Download Started..........");
			String strNewTransactionPath=	TTKPropertiesReader.getPropertyValue("path.GN.dhpoClaimDownload");
		...
		log.info("DHPO GlobalNet Claim Download Ended..........");
			}
		}catch(Exception e){
			try{
				fileWriter.write("Error Occurred::"+e.getMessage());
			...
			new Exception(strDHPOGlobalNetClaimDownloadError);
	...
	private MaintenanceManager getMaintenanceManagerObject() throws TTKException
	{
		...
		{
			throw new TTKException(exp, strDHPOGlobalNetClaimDownloadError);
		}//end of catch
