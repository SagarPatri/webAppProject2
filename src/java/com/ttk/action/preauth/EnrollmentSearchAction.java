...
public class EnrollmentSearchAction extends TTKAction
{
	...
	private static final String strClaimList="claimlist";
	...
	private static final String strClaimEnrollmentList="claimenrollmentlist";
	...
	//Hospital Login 
	private static final String strHospitalLogin="Hospital Information";
	private static final String strHospPreAuth="Pre-Auth";
	private static final String strHospClaim="Claims";
	private static final String strHospPreAuthenrollmentList="hosppreauthenrollmentlist";
	private static final String strHospClaimEnrollmentList="hospclaimenrollmentlist";
	private static final String strHospPreAuthDetail="hosppreauthdetail";
	private static final String strHospClaimdetails="hospclaimsdetail";
	...
	public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			...
			String strEnrollmentList="";
			 strEnrollmentList =("BatchEntry".equals(TTKCommon.getActiveSubLink(request))?"claimBatchDetails": this.getForwardPath(request));
	...
	public ActionForward doSearch(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			...
			else if(TTKCommon.getActiveLink(request).equalsIgnoreCase(strHospitalLogin))
			{
				
				if(TTKCommon.getActiveSubLink(request).equals("Pre-Auth") && TTKCommon.getActiveSubLink(request).equals("Claims"))
				{
				alEnrollment = PreAuthManagerObject.getEnrollmentList(tableData.getSearchData());
				}
			}else // end of if(TTKCommon.getActiveLink(request).equalsIgnoreCase("strHospitalLogin"))			
			{
	...
	public ActionForward doClose(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		{
			setLinks(request);
			log.info("Inside doSearch doClose");
			String strEnrollmentList="";
			 strEnrollmentList =("BatchEntry".equals(TTKCommon.getActiveSubLink(request))?"claimBatchAddDetails": this.getForwardPath(request));
			 ((DynaActionForm)form).initialize(mapping);
			 //finally return to the grid screen
			 
			 //To display correct screen in call center feature closing of enrollment window.
			 if(("Call Center").equals(TTKCommon.getActiveSubLink(request))){
				 if(("Call Details").equals(TTKCommon.getActiveTab(request)))
					 strEnrollmentList=strCallcenterDetails;
				 
				 if(("Search").equals(TTKCommon.getActiveTab(request)))
					 strEnrollmentList = strCallcenterSearch;
			 }
	...
	public ActionForward doSelectEnrollment(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			...
			DynaActionForm frmHospPreAuthGeneral=null;//added asper Hospital Login
			String strGeneralDetails="";
			String strSubLink=TTKCommon.getActiveSubLink(request);
			String strActiveLink=TTKCommon.getActiveLink(request);
			String strActiveTab=TTKCommon.getActiveTab(request);
			Toolbar toolBar = (Toolbar)request.getSession().getAttribute("toolbar");
			if("BatchEntry".equals(strSubLink)){
			DynaActionForm 	frmAddClaimDetails=(DynaActionForm)request.getSession().getAttribute("frmAddClaimDetails");	
			ClaimantNewVO claimantNewVO = (ClaimantNewVO)((TableData)request.getSession().getAttribute("tableData")).getRowInfo(Integer.parseInt(request.getParameter("rownum")));
			frmAddClaimDetails.set("enrollmentSeqID", claimantNewVO.getMemberSeqID().toString());
			frmAddClaimDetails.set("enrollmentID", claimantNewVO.getEnrollmentID());
			String claimType	=	frmAddClaimDetails.getString("claimType");
			String hospSeqId	=	frmAddClaimDetails.getString("providerID");
			String partnerName	=	frmAddClaimDetails.getString("partnerName");
			String submissionCatagory	=	frmAddClaimDetails.getString("processType");
			String paymentTo	=	frmAddClaimDetails.getString("paymentTo");
			if("PTN".equals(paymentTo))
				hospSeqId	=	partnerName;
			
			Map<String,String> previousClaimNums=claimManagerObject.getMemClaimList(claimantNewVO.getMemberSeqID(),claimType,hospSeqId,submissionCatagory,paymentTo);
			...
			((DynaActionForm)form).initialize(mapping);
			 return this.getForward("claimBatchAddDetails", mapping, request);
			}
			...
			//added as per Hospital login
			if(strActiveLink.equals(strHospitalLogin))
			{
				if(strActiveTab.equals(strHospPreAuth))
				{
					strGeneralDetails=strHospPreAuthDetail;
				}//end of if(strActiveTab.equals(strSearch))
				else if(strActiveTab.equals(strHospClaim))
				{
					strGeneralDetails=strHospClaimdetails;
				}//end of else if(strActiveTab.equals(strCallDetails))
			}//end of if(strActiveLink.equals(strCustomerCare))
			//added as per Hospital login
			...
			if(!(TTKCommon.checkNull(request.getParameter("rownum")).equals("")))
			{
				...
				//...............................................................added as per Hospital login ..................................
				frmHospPreAuthGeneral=(DynaActionForm)request.getSession().getAttribute("frmPreAuthGeneral");
				if(!(TTKCommon.checkNull(request.getParameter("rownum")).equals("")))
				{
					if(TTKCommon.getActiveLink(request).equals("Pre-Authorization") && TTKCommon.getActiveLink(request).equals("Claims"))
					{
					...
					if(strSubLink.equals(strPreAuthProcessing))
					{
						frmHospPreAuthGeneral.set("frmChanged","changed");
						if(strActiveLink.equals(strClaims))
						{
							if(((frmHospPreAuthGeneral.get("authNbr") != null && (!"".equals(frmHospPreAuthGeneral.get("authNbr")))) || (frmHospPreAuthGeneral.get("prevHospClaimSeqID") != null)))
							{
								clearPreauthHospital(mapping,frmHospPreAuthGeneral,request,claimManagerObject);
								HashMap hmPrevHospList = new HashMap();
								//adding hmPrevHospList while selecting new enrollmentid
								if(ClaimsWebBoardHelper.getMemberSeqId(request) != null && !"".equals(ClaimsWebBoardHelper.getMemberSeqId(request)))
								{
									DynaActionForm frmClaimantDetails=(DynaActionForm)frmHospPreAuthGeneral.get("claimDetailVO");
									hmPrevHospList = claimManagerObject.getPrevHospList(ClaimsWebBoardHelper.getMemberSeqId(request),frmClaimantDetails.getString("claimTypeID"),ClaimsWebBoardHelper.getClaimsSeqId(request));
								}//end of if
								frmHospPreAuthGeneral.set("hmPrevHospList",hmPrevHospList);
							}//end of if 
						}//end of if(strActiveTab.equals(strClaims))
						//if pre-auth process is completed, link only Enrollment-Id
						if(frmHospPreAuthGeneral.get("completedYN").equals("Y"))
						{
							DynaActionForm frmClaimantDetails=(DynaActionForm)frmHospPreAuthGeneral.get("claimantDetailsVO");
							frmClaimantDetails.set("enrollmentID",claimantVO.getEnrollmentID());
							frmClaimantDetails.set("policyNbr",claimantVO.getPolicyNbr());
							frmClaimantDetails.set("memberSeqID",claimantVO.getMemberSeqID().toString());
							frmClaimantDetails.set("policySeqID",claimantVO.getPolicySeqID().toString());
							...
							//						Convert it regular Preauth, if current status is Manual
							if(("MAN").equals(frmHospPreAuthGeneral.get("preAuthTypeID")))
							{
								frmHospPreAuthGeneral.set("preAuthTypeID","MRG");//change the pre-auth type to regular.
								frmHospPreAuthGeneral.set("preAuthTypeDesc","Manual-Regular");
							}//end of if(("MAN").equals(frmHospPreAuthGeneral.get("preAuthTypeID")))
						}//end of if(frmHospPreAuthGeneral.get("completedYN").equals("Y"))
						//if pre-auth is under process, fetch all the data related to that particular enrollment
						else if(frmHospPreAuthGeneral.get("completedYN").equals("N"))
						{

							frmHospPreAuthGeneral.set("claimantDetailsVO",FormUtils.setFormValues("frmClaimantDetails",claimantVO,
									this,mapping,request));

							//Convert it regular Preauth, if current status is Manual
							if(("MAN").equals(frmHospPreAuthGeneral.get("preAuthTypeID")))
							{
								frmHospPreAuthGeneral.set("preAuthTypeID","REG");//change the pre-auth type to regular.
								frmHospPreAuthGeneral.set("preAuthTypeDesc","Regular");
							}//end of if(("MAN").equals(frmHospPreAuthGeneral.get("preAuthTypeID")))

						}//end of else if(frmHospPreAuthGeneral.get("completedYN").equals("N"))
					}//end of if(strSubLink.equals(strPreAuthProcessing))
				}
					//added as per Hospital Login
				...
				if(strActiveLink.equals(strCustomerCare))
				{
					...
					else if(strActiveTab.equals(strCallDetails))
					{
						...
						if(frmCallCenterDetails != null)
						{
							...
							frmCallCenterDetails.set("age",claimantVO.getAge().toString());//KOC FOR Grievance
							frmCallCenterDetails.set("gender",claimantVO.getGenderDescription());//KOC FOR Grievance
	...
	private String getForwardPath(HttpServletRequest request) throws TTKException{
		...
		try{
			...
			if(strActiveLink.equals(strHospitalLogin))
			{
				if(strSubLink.equals(strHospPreAuth))
				{
					strEnrollmentList=strHospPreAuthenrollmentList;
				}//end of if(strActiveTab.equals(strSearch))
				else if(strSubLink.equals(strHospClaim))
				{
					strEnrollmentList=strHospClaimEnrollmentList;
				}//end of else if(strActiveTab.equals(strCallDetails))
			}//end of if(strActiveLink.equals(strCustomerCare))
	...
	throws TTKException
	{
		...
		else
		{
			...
			alSearchParams.add((String)frmEnrollSearch.get("sQatarId"));
