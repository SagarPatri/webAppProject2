...
/**
 * @ (#) CanvalescenceBenefitAction.java   MAY 20, 2013
 * Project        : TTK HealthCare Services
 * File           : CanvalescenceBenefitAction.java
 * Author         : 
...
public class CanvalescenceBenefitAction extends TTKAction
{
	private static Logger log = Logger.getLogger( CanvalescenceBenefitAction.class );

	//Action mapping forwards.
	private static final String strProductCanvalescenceBenefitList="CanvalescenceBenefitlist";
	private static final String strPolicyCanvalescenceBenefitList="policyCanvalescenceBenefitlist";
	...
	public ActionForward  doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)throws Exception 
	{
		try
		{
			log.debug("Inside the doDefault method of CanvalescenceBenefitAction");
			setLinks(request);
			String strTable = "";
			DynaActionForm frmCanvalescenceBenefitList =(DynaActionForm)form;
			...
			if("Products".equals(strActiveSubLink)){
				strForwardPath=strProductCanvalescenceBenefitList;
			}//end of if("Products".equals(strActiveSubLink))
			else {
				strForwardPath=strPolicyCanvalescenceBenefitList;
			}//end of else
			frmCanvalescenceBenefitList.set("caption",sbfCaption.toString());
			...
			strTable = "CanvalescenceBenefitListTable";
			...
			alPlanList = productPolicyConfig.getCanvalescenceBenefitList(tableData.getSearchData());
			tableData.setData(alPlanList,"search");
			request.getSession().setAttribute("tableData",tableData);
			request.setAttribute("frmCanvalescenceBenefitList",frmCanvalescenceBenefitList);
	...
	public ActionForward doBackward(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try{
			log.debug("Inside the doBackward method of CanvalescenceBenefitAction");
			setLinks(request);
			String strForwardPath=null;
			String strActiveSubLink=TTKCommon.getActiveSubLink(request);
			ProdPolicyConfigManager  productPolicyConfig= null;
			...
			ArrayList alPlanList = productPolicyConfig.getCanvalescenceBenefitList(tableData.getSearchData());
	...
	public ActionForward doForward(ActionMapping mapping,ActionForm form,HttpServletRequest request,
													 HttpServletResponse response) throws TTKException{
		try{
			log.debug("Inside the doForward method of CanvalescenceBenefitAction");
			setLinks(request);
			String strForwardPath=null;
			String strActiveSubLink=TTKCommon.getActiveSubLink(request);
			ProdPolicyConfigManager  productPolicyConfig= null;
	...
	public ActionForward doClose(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			log.debug("Inside the doClose method of CanvalescenceBenefitAction");
			setLinks(request);
	...
	private ArrayList<Object> populateSearchCriteria(HttpServletRequest request) throws TTKException
	{
		...
		Long lnginsConvBenfSeqID=TTKCommon.getWebBoardId(request);
		Long lngUserSeqId=TTKCommon.getUserSeqId(request);
		alSearchParams.add(new Long(lnginsConvBenfSeqID));
		
		String strconv=TTKCommon.getCashBenefitID(request);
		...
		alSearchParams.add(new String(strconv));
