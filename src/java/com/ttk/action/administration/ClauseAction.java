/**
 * @ (#) ClauseAction.java July 9th, 2007
 * Project      : TTK HealthCare Services
 * File         : ClauseAction.java
 * Author       : Krupa J
 * Company      : Span Systems Corporation
 * Date Created : July 9th, 2007
 *
...
public class ClauseAction extends TTKAction
{
	private static Logger log = Logger.getLogger(ClauseAction.class);

	//for forwarding
	private static final String strClauses="clauses";
	private static final String strDeleteClauses="deleteClauses";
	private static final String strPolicyClauses="policyClauses";
	private static final String strDelPolicyClauses="delPolicyClauses";
	...
	public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		{
			setLinks(request);
			log.debug("Inside ClauseAction doDefault");
			...
			DynaActionForm frmClauses = (DynaActionForm)form;
			...
			tableDataClause.createTableInfo("ClauseTable",null);
			...
			frmClauses.set("caption",buildCaption(request));
	 ...
	 * @throws Exception if any error occurs
	 */
	public ActionForward doViewClause(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		try
		{
			setLinks(request);

			log.debug("Inside ClauseAction doViewClause");
			...
			if(!(TTKCommon.checkNull(request.getParameter("rownum")).equals("")))
			{
				...
				frmClauses = (DynaActionForm)FormUtils.setFormValues("frmClauses",policyClauseVO,this,mapping,request);

			}// end of if(!(TTKCommon.checkNull(request.getParameter("rownum")).equals("")))
			else
			{
				frmClauses.initialize(mapping);
			}// end of else
			frmClauses.set("caption",buildCaption(request));
			request.getSession().setAttribute("frmClauses",frmClauses);
			return this.getForward(strForward, mapping, request);
		}//end of try
		catch(TTKException expTTK)
		{
	...
	}//end of doViewClause(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse
	...
	public ActionForward doSave(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		{
			setLinks(request);
			log.debug("Inside ClauseAction doSave");
			PolicyClauseVO policyClauseVO =null;
			DynaActionForm frmClauses=(DynaActionForm)form;
			DynaActionForm frmProductList = null;
			DynaActionForm frmPoliciesSearch = null;
			...
			ArrayList alClauseList=null;
			policyClauseVO=(PolicyClauseVO)FormUtils.getFormValues(frmClauses,this, mapping, request);
			policyClauseVO.setProdPolicySeqID(TTKCommon.getWebBoardId(request));
			policyClauseVO.setClauseYN("Y");
	...
	public ActionForward doReset(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		{
			setLinks(request);
			log.debug("Inside ClauseAction doReset");
			String strActiveSubLink=TTKCommon.getActiveSubLink(request);
			String strForward="";
			if(strActiveSubLink.equals("Products"))
			{
	...
	public ActionForward doDelete(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws Exception
	{
		...
		{
			setLinks(request);
			log.debug("Inside ClauseAction doDelete");
			String strActiveSubLink=TTKCommon.getActiveSubLink(request);
			DynaActionForm frmClauses=(DynaActionForm)form;
			DynaActionForm frmProductList = null;
			DynaActionForm frmPoliciesSearch = null;
			...
			{
				frmProductList = (DynaActionForm)request.getSession().getAttribute("frmProductList");
				strForward=strDeleteClauses;
			}//end of if(strActiveSubLink.equals("Products"))
			else if(strActiveSubLink.equals("Policies"))
			{
				frmPoliciesSearch = (DynaActionForm)request.getSession().getAttribute("frmPoliciesSearch");
				strForward=strDelPolicyClauses;
			}//end of else if(strActiveSubLink.equals("Policies"))
	...
	private String buildCaption(HttpServletRequest request)throws Exception
	{
		...
		if(strActiveSubLink.equals("Products"))
		{
			...
			sbfCaption.append("List of Clauses - [ ").append(frmProductList.get("companyName")).append(" ] [ ").append(frmProductList.get("productName")).append(" ]");
		}//end of if(strActiveSubLink.equals("Products"))
		else if(strActiveSubLink.equals("Policies"))
		{
			DynaActionForm frmPolicies = (DynaActionForm)request.getSession().getAttribute("frmPoliciesEdit");
			sbfCaption.append("List of Clauses - [ ").append(frmPolicies.get("companyName")).append(" ] [ ").append(frmPolicies.get("policyNbr")).append(" ]");
		}//end of else if(strActiveSubLink.equals("Policies"))
