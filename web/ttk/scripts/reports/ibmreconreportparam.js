...
function onSelectGroup()
{
	...
	document.forms[1].reportID.value="MontlyRecon";
	document.forms[1].action="/IBMReportsAction.do";
	document.forms[1].submit();
...
function onSelectPolicy()
{
	if(document.forms[1].policySeqID.value!=""){
		document.forms[1].mode.value="doIBMReconSelectPolicy";
		document.forms[1].action="/IBMReportsAction.do";
		document.forms[1].submit();
...
function onGenerateReconReport()
{
	var groupID = document.forms[1].groupId.value;
	var policy = document.forms[1].policySeqID.value;
	...
	//var partmeter = "?mode=doGenerateReconReport &parameter="+parameterValue+"&fileName=reports/enrollment/AccentureReport.jrxml&reportID=MontlyRecon&reportType="+reportType
					//+"&sFrom="+reportFrom+"&sTo="+reportTo;
    var partmeter = "?mode=doGenerateReconReport&Polgrpseqid="+policySeqID+"&Policy="+policy+"&reportID=MontlyRecon&reportType="+reportType+"&sFrom="+reportFrom+"&sTo="+reportTo;
	var openPage = "/IBMReportsAction.do"+partmeter;
	var w = screen.availWidth - 10;
	var h = screen.availHeight - 99;
	var features = "scrollbars=0,status=1,toolbar=1,top=0,left=0,resizable=0,menubar=yes,width="+w+",height="+h;
	document.forms[1].mode.value="doGenerateReconReport ";
	window.open(openPage,'',features);
	//document.forms[1].submit();
}//end of onGenerateReconReport 
