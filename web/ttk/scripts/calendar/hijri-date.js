...
Date.prototype.getDaysInMonth = function() {
	var y = this.getHFullYear(),
		isLeapYear = (y % 100 !== 0) && (y % 4 === 0) || (y % 400 === 0),
		daysInMonth = [31, isLeapYear ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
	return daysInMonth[this.getHMonth()];
};

Date.prototype.getMonthName = function(month) {
	return Date.monthNames[(month === undefined) ? this.getHMonth() : parseInt(month)];
};

Date.prototype.getMonthShortName = function(month) {
	return Date.monthShortNames[(month === undefined) ? this.getHMonth() : parseInt(month)];
};
