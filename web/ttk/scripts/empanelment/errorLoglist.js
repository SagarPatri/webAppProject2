...
function pageIndex(pagenumber)
{
    ...
    document.forms[1].action = "/TariffViewLogListAction.do";
...
function onSearch()
{
   if(!JS_SecondSubmit)
 {
	...
	if (fromDate.length>1 && !pattern.test(fromDate)){
		...
		document.forms[1].formDate.value= "";
		return;
 }
...
{
    var openPage = "/TariffViewLogListAction.do?mode=doViewErrorLog&reportType=EXCEL&rownum="+rownum;
