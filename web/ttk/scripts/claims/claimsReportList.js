function toggle(sortid)
{
    ...
    document.forms[1].mode.value="doClmDetailedReportSearch";
    document.forms[1].sortId.value=sortid;
    if(!JS_SecondSubmit){
    document.forms[1].action = "/ClaimDetailedReports.do";
...
function PressBackWard()
{
	if(!JS_SecondSubmit){
    document.forms[1].reset();
    document.forms[1].mode.value ="doClmDetailedReportBackward";
    document.forms[1].action = "/ClaimDetailedReports.do";
    JS_SecondSubmit=true;
    document.forms[1].submit();
	}
...
function PressForward()
{
	if(!JS_SecondSubmit){
    document.forms[1].reset();
    document.forms[1].mode.value ="doClmDetailedReportForward";
    document.forms[1].action = "/ClaimDetailedReports.do";
	 JS_SecondSubmit=true;
    document.forms[1].submit();
	}
   ...
   if(!JS_SecondSubmit)
 {
	...
	if((document.forms[1].patientName.value!=""  || document.forms[1].invoiceNo.value!="" ||
			...
			document.forms[1].qatarId.value!="" || document.forms[1].payRefNo.value!=""))
		{
			...
			if(document.forms[1].switchType.value == 'QPR')
			{
				if(document.forms[1].sProviderName.value==""){
					alert("Please Select Provider Name.");
					document.forms[1].sProviderName.focus();
					return false;
				}	
			}
...
function dateValidation(stParts,endParts,fromDateVar,toDateVar){
	...
	{
		document.forms[1].action="/ClaimDetailedReports.do";
		document.forms[1].mode.value="doChangeSwitchType";
		document.forms[1].submit();
	}// end of onSwitch()
	...
	function onExportToResubmissionExcel()
	{
		...
		var validateFlag=dateValidation(stParts,endParts,'Treatment Start date','Treatment End date');
			if(validateFlag==true){
				var stParts =document.forms[1].clmFromDate.value.split('/');
				var endParts=document.forms[1].clmToDate.value.split('/');
				var validateFlag=dateValidation(stParts,endParts,'Claim Re-Submission From Date','Claim Re-Submission To Date');
				if(validateFlag==true){
					var trtmtStartDate=document.forms[1].tmtfromDate.value;
					var trtmtEndDate=document.forms[1].tmttoDate.value;
					var clmFromDate=document.forms[1].clmFromDate.value;
					...
					var status=document.forms[1].claimStatus.value;
					var batchNumber=document.forms[1].batchNo.value;
					var batchRefNo=document.forms[1].batchRefNo.value;
					var claimNumber=document.forms[1].claimNo.value
					...
					parameterValue =  "|"+trtmtStartDate+"|"+trtmtEndDate+"|"+clmFromDate+"|"+clmToDate+"|"+alKootId+"|"+sProviderName+"|"+status+"|"+batchNumber+"|"+batchRefNo+"|"+claimNumber+"|";	
					
					
					var partmeter = "?mode=doGenerateClmResubmissionReport&parameter="+parameterValue;
				   	var openPage = "/ClaimReSubmissionDetailedReports.do"+partmeter;
					var w = screen.availWidth - 10;
					var h = screen.availHeight - 99;
					var features = "scrollbars=0,status=1,toolbar=1,top=0,left=0,resizable=0,menubar=yes,width="+w+",height="+h;
					window.open(openPage,'',features);   
