	...
	if(!JS_SecondSubmit)
	 {
		...
		document.forms[0].action = "/OnlineCashlessPartAction.do";
		JS_SecondSubmit=true
...
function onValidateEnroll()
{
	...
	if(!JS_SecondSubmit)
	{
		...
		document.forms[1].action = "/OnlineCashlessPartActionNew.do?&benifit="+selText+"";
		JS_SecondSubmit=true;
		document.forms[1].submit();
	}//end of if(!JS_SecondSubmit)
...
function onCheckEligibility()
{
	var selIndex = document.forms[1].benifitTypeID.selectedIndex;
	var selText = document.forms[1].benifitTypeID.options[parseInt(selIndex)].text;
	var encIndex = document.forms[1].encounterTypeIndex.selectedIndex;
	var encText = document.forms[1].ptrEncounterType.options[parseInt(encIndex)].text;
	var benifitTypeID	=	document.getElementById("benifitTypeID").value;
	if(!JS_SecondSubmit)
	{
		trimForm(document.forms[1]);
		document.forms[1].mode.value="doValidate";
		document.forms[1].action = "/OnlineCashlessPartActionNew.do?&benifit="+selText+"&benifitTypeID="+benifitTypeID+"&encounter="+encText;
		JS_SecondSubmit=true;
		document.forms[1].submit();
	}//end of if(!JS_SecondSubmit)
}//end of onValidateEnroll()
...
{	alert("doSubmitTests");
		...
		document.forms[1].action ="/OnlineCashlessPartAction.do";
		document.forms[1].submit();
...
{	
	...
	document.forms[0].action = "/OnlineCashlessPartAction.do?sOffIds="+offids+"";
...
function onSubmitActualRate()
{
	...
	//document.getElementById("sOffIds").value=offids;
	//document.forms[0].sOffIds.value = offids;	
	document.forms[0].action = "/OnlineCashlessPartAction.do?reqAmnt="+reqAmnt+"&reqAmnts="+reqAmnts;
...
function onConfirmOtp()
{
	...
	if(!JS_SecondSubmit)
	{
		...
		document.forms[0].action = "/OnlineCashlessPartAction.do?otp="+otp;
		JS_SecondSubmit=true;
		document.forms[0].submit();
	}//end of if(!JS_SecondSubmit)
}
...
function onPolicyTob()
{
	var prodPolicySeqId=document.getElementById("prodPolicySeqId").value;
	   ...
	   //	alert("hello::::"+claimSeqID);
	   	var openPage = "/ViewTOBOnlineCashlessPartAction.do?mode=doViewPolicyTOB&policySeqId="+prodPolicySeqId;	 
	   	var w = screen.availWidth - 10;
	  	var h = screen.availHeight - 49;
	  	var features = "scrollbars=0,status=1,toolbar=0,top=0,left=0,resizable=0,menubar=yes,width="+w+",height="+h;
	  	window.open(openPage,'',features);
...
function onBenefitsDisplay()
{
	...
	document.forms[1].action="/OnlineCashlessPartAction.do?policySeqId="+policySeqId+"&memberSeqId="+memberSeqId+"&benefitType4=OPT";
	JS_SecondSubmit=true;
	document.forms[1].submit();	
	}
}
