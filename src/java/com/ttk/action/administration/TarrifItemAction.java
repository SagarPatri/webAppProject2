...
public class TarrifItemAction extends TTKAction
{
	...
	private static final String strTariffSearchEmpanelmentInSwitch		=	"searchTariffEmapanelment";
	...
	private static final String strmodifytariff="modifytariffEmpanelment";
	private static final String strSearchErrorLog  = "searchErrorLogTariff";
	...
	public ActionForward doEditTariff(ActionMapping mapping,ActionForm form,HttpServletRequest request,
    	...
    	try{
			...
			if(!(switchTo==null||switchTo.trim().equals("")))
				request.setAttribute("switchTo",switchTo);
            ...
            else
            {
          	...
          	request.getSession().setAttribute("tariffUploadVO", tariffUploadVO);
            ...
            TariffItemForm.set("internalCodeDesc",tariffUploadVO.getInternalCodeDesc());
            ...
            request.getSession().setAttribute("isActCodeChanged","false");
            if("Tariff".equals(TTKCommon.getActiveSubLink(request)))
...
//			String activityCode=request.getParameter("activityCode");
			String isActCodeChanged=null;
			if(request.getSession().getAttribute("isActCodeChanged")!=null)
			isActCodeChanged=(String) request.getSession().getAttribute("isActCodeChanged") ;
...
/*			if(isActCodeChanged==false){
				if(!activityCode.equals("")){
					ArrayList<ActivityDetailsVO> alActivityCodeList=null;
					TableData activityCodeListData = new TableData();;
					activityCodeListData.setSearchData(this.populateActivityCodeSearchCriteria(activityCode,request));
					alActivityCodeList= tariffItemObject.getActivityCodeList(activityCodeListData.getSearchData(),switchToVal);
					activityCodeListData.setData(alActivityCodeList, "search");
					ActivityDetailsVO activityDetailsVO = alActivityCodeList.get(0);
					...
					tariffUploadVO.setServiceSeqId(Long.parseLong(activityDetailsVO.getServiceInternalCode()));
				}
			}
			request.getSession().setAttribute("isActCodeChanged",false);
			*/
			String remark=tariffUploadVO.getRemarks();
			long lngCount=0;
			if(isActCodeChanged!=null&&isActCodeChanged.equals("true")){
				if(remark!=null&&!remark.equals("")){
					lngCount=tariffItemObject.editTariffItem(tariffUploadVO,(TTKCommon.getUserSeqId(request)));
				}else
				request.setAttribute("errorMsg", "Remarks is Required");
			}else{
				lngCount=tariffItemObject.editTariffItem(tariffUploadVO,(TTKCommon.getUserSeqId(request)));
			...
			frmTariffItem.set("strInternalCode", tariffUploadVO.getInternalCode());
			request.getSession().setAttribute("tariffUploadVO",tariffUploadVO);
    ...
    public ActionForward doShowTariff(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			...
			try{
				...
				((DynaActionForm)form).initialize(mapping);//reset the form data
				request.getSession().setAttribute("switchTo", "ACT");
				if("Tariff".equals(TTKCommon.getActiveSubLink(request)))
					return this.getForward(strShowrTariffs, mapping, request);
				else{
    ...
    public ActionForward doSwitch(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			...
			try{
				...
				if("PAC".equals(switchType)){
					...
					return mapping.findForward(strPharmacyTariffSearchEmpanelment);
				}else{
					request.getSession().setAttribute("switchToVal", "ACTIVITY TARIFF");
					return mapping.findForward(strTariffSearchEmpanelmentInSwitch);
				}
			}//end of try
			catch(Exception exp)
			{
	...
	public ActionForward doSearchTariffItems(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			...
			try{
				...
				TariffManager tariffItemObject=this.getTariffItemObject();						
				TableData tableData = TTKCommon.getTableData(request);
				String switchTo = (String)request.getParameter("switchTo");
				request.setAttribute("switchTo", switchTo);
			//clear the dynaform if visiting from left links for the first time
			//else get the dynaform data from session
			if(TTKCommon.checkNull(request.getParameter("Entry")).equals("Y"))
	...
	private ArrayList<Object> populateActivityCodeSearchCriteria(String activityCode,HttpServletRequest request)
	{
		//build the column names along with their values to be searched
		ArrayList<Object> alSearchParams = new ArrayList<Object>();
		alSearchParams.add(TTKCommon.replaceSingleQots(activityCode));
		alSearchParams.add(TTKCommon.replaceSingleQots(""));
		alSearchParams.add(TTKCommon.replaceSingleQots(""));
		alSearchParams.add(TTKCommon.replaceSingleQots(""));
		alSearchParams.add(TTKCommon.replaceSingleQots(""));
		alSearchParams.add(TTKCommon.replaceSingleQots("1"));
		alSearchParams.add(TTKCommon.replaceSingleQots("1"));
	  ...
	  try{   
		  log.info("Inside the doModifySaveServiceTypes method of TarifItemAction");
	    	...
	    	String strFileerror = TTKPropertiesReader.getPropertyValue("GlobalDownload")+"/fileImproper.pdf";
			  ...
			  Object[] objects	=	tariffItemObject.getTariffUploadedData(hospSeqId);
			  ...
			  ArrayList<String[]> alErrorMsg	=	new ArrayList<String[]>();
	            alErrorMsg	=	(ArrayList<String[]>) objects[0];
	 	           ...
	 	           response.setHeader("Content-Disposition", "attachment; filename=TariffDetails.xls");
	 	            errWorkbook		=	createTariffDownloadFile(errWorkbook,alErrorMsg);
	...
	public ActionForward doGeneral(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			return mapping.findForward("activitySearchList");
		}// end of try
		catch (Exception exp) {
	...
	public ActionForward doShowBulkModify(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			HttpServletResponse response) throws Exception{
		try{
		log.debug("Inside the doShowBulkModify method of TarifItemAction");
		setLinks(request);
		DynaActionForm frmSearchTariffItem	=	(DynaActionForm)form;
		((DynaActionForm)form).initialize(mapping);//reset the form data
//		String switchToVal= (String) frmSearchTariffItem.get("switchTo");
		String switchToVal= (String) request.getSession().getAttribute("switchToVal");
		if(switchToVal==null||switchToVal.equals("")|| switchToVal.equals("ACTIVITY TARIFF")){
			request.getSession().setAttribute("switchToVal", "ACTIVITY TARIFF");
		}else if(switchToVal.equals("PHARMACY/CONSUMABLE TARIFF")){
			request.getSession().setAttribute("switchToVal", "PHARMACY/CONSUMABLE TARIFF");
		}
		return this.getForward(strmodifytariff, mapping, request);//
	...
	private HSSFWorkbook createTariffDownloadFile(HSSFWorkbook errWorkbook,ArrayList<String[]> alErrorMsg){
		   HSSFSheet errSheet			=	errWorkbook.createSheet("ErrorLog"); 
			...
			style.setFont(font);
				...
				errRow.createCell((short) 5).setCellValue("Package_Id");
				errRow.createCell((short) 6).setCellValue("Bundle_Id");
				errRow.createCell((short) 7).setCellValue("Gross_Amount");
	...
	public ActionForward doShowErrorLogTariff(ActionMapping mapping,ActionForm form, HttpServletRequest request,HttpServletResponse response) throws Exception {
		try {
		log.info("Inside the doShowErrorLogTariff method of TariffAction");
		setLinks(request);
		...
		tableData.createTableInfo("TariffUploadErrorLogList",new ArrayList());
		session.setAttribute("tableData",tableData);
		frmErrorLogList.initialize(mapping);//reset the form data
	    return mapping.findForward(strSearchErrorLog);
		}// end of try
		catch (TTKException expTTK) {
		return this.processExceptions(request, mapping, expTTK);
		}// end of catch(TTKException expTTK)
