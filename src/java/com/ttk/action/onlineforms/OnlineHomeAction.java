/**
 * @ (#) OnlineHomeAction.java Dec 26,2007
 * Project       : TTK HealthCare Services
 * File          : OnlineHomeAction.java
 ...
 */
public class OnlineHomeAction extends TTKAction {
	private static Logger log = Logger.getLogger( OnlineHomeAction.class );
	...
	public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try{
			...
			log.info("Inside OnlineHomeAction doDefault ");
			...
			request.getSession().setAttribute("loginType", strLoginType);
			//  
			//homebroker
			...
			if(strLoginType.equals("H"))
			{
				...
				hmCorporateDetails=onlineAccessManagerObject.getCorporateDetails(strUserID);
				
				request.getSession().setAttribute("hmCorporateDetails",hmCorporateDetails);
			...
			else if(strLoginType.equals("PTR"))//added as per Hospital login
			{
				alResult = onlineAccessManagerObject.getOnlineHomeDetails(strGroupID,null,new Long(0),"PTR",strUserID);
			}//end of //added as per partner login
			else 
			if(!strLoginType.equals("EMPL"))
			{
//				alResult = onlineAccessManagerObject.getOnlineHomeDetails("A0013","1/BHAGAVAN",new Long(0),"OCO","1");
			...
			/*if(strLoginType.equals("EMPL")){
				request.getSession().setAttribute("onlinehome",onlinehome);
			}*/
			...
			{				
			 if(strLoginType.equals("PTR"))
				return this.getForward("onlinepartnerhome", mapping, request);
			 else if("EMPL".equals(strLoginType))
		...
		catch(Exception exp)
		{
			if("EMPL".equals(strLoginType))
				return this.processExceptions(request, mapping, new TTKException(exp, "onlineEmpHome"));
			else
			return this.processExceptions(request, mapping, new TTKException(exp, strOnlineHome));
...
//			      build the column names along with their values to be searched
	...
	public ActionForward doSelectPolicy(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try{
			...
			Document onlinehome=null;
			ArrayList<Object> alResult = new ArrayList<Object>();
			log.info("Inside OnlineHomeAction doSelectPolicy");
...
private String appendURlSSRSReport(String str){
	...
	public ActionForward doHRProceed(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			HttpServletResponse response) throws Exception{
		...
		try{ 
			...
			log.info("Inside OnlineHomeAction doHRProceed ");
			//finally return to the grid screen
			return mapping.findForward("EnrollmentDashedBoard");
		}//end of try
		catch(TTKException expTTK)
		{
			return this.processOnlineExceptions(request, mapping, expTTK);
	...
	public ByteArrayOutputStream generateReport(JasperPrint jasperPrint1, JasperPrint jasperPrint2) throws JRException {
		  ...
		  //this will make a book mark in the exported PDF for each of the reports
