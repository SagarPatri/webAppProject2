...
public class MemberDAOImpl implements BaseDAO, Serializable {
    ...
    private static final String strSaveMember = "{CALL POLICY_ENROLLMENT_PKG.SAVE_POLICY_MEMBER(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"; // added by vishwanath
    private static final String strGetRelationshipCode = "SELECT A.RELSHIP_TYPE_ID || '#' || A.ASSOC_GENDER_REL RELSHIP,A.RELSHIP_DESCRIPTION FROM TPA_RELATIONSHIP_CODE A ORDER BY A.SORT_NO";
    ...
    private static final String strSaveEnrollment = "{CALL POLICY_ENROLLMENT_PKG.SAVE_POLICY_GROUP(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";//one extra parameter added by Praveen for emailId2-KOC-1216
 ...
 //   private static final String strSaveGroupPremium = "{CALL POLICY_ENROLLMENT_PKG.SAVE_GROUP_PREMIUM(?,?,?,?,?,?,?,?,?,?)}";
    private static final String strSaveGroupPremium = "{CALL POLICY_ENROLLMENT_PKG.SAVE_GROUP_PREMIUM(?,?,?,?,?,?,?,?,?,?,?)}";//modified as per KOC 1284 Change request
...
//    private static final String strSaveMemDomiciliary = "{CALL POLICY_ENROLLMENT_PKG.SAVE_MEM_DOMICILARY(?,?,?,?,?,?,?)}";
    ...
    private static final String strRenewalPolicyStatus = "{CALL POLICY_ENROLLMENT_PKG.CHECK_REN_OR_FRS(?,?)}";
    ...
    private static final String strGetMemberMedicalCertificate = "{CALL POLICY_ENROLLMENT_PKG.dwnld_travelcertificate(?,?)}";
    private static final String strGetMemPolicyRule = "{CALL POLICY_ENROLLMENT_PKG.select_member_rulelist(?,?)}";
    private static final String strSaveMemberMdf = "{CALL POLICY_ENROLLMENT_PKG.SAVE_MEM_MDF(?,?,?,?,?,?)}";
    private static final String strUploadedMdfFilesList = "{CALL POLICY_ENROLLMENT_PKG.SELECT_MEM_MDF_LIST(?,?,?,?)}";
    private static final String strDeleteMemberMdf = "{CALL POLICY_ENROLLMENT_PKG.DELETE_MEM_MDF(?,?,?,?)}";
    ...
    private static final int v_net_prem = 74;
    private static final int v_hijri_dob = 75;
    private static final int v_stopclaimsYN = 76;
    private static final int v_stopclaimsDate = 77; 
    private static final int MAIL_NOTIFICATION_FLAG = 78;
    ...
    public ArrayList getMemberList(ArrayList alSearchCriteria) throws TTKException
    {
        ...
        try {
            ...
            if(alSearchCriteria.get(10)!=null){
                cStmtObject.setInt(10,new Integer((String)alSearchCriteria.get(10)));
            ...
            if(alSearchCriteria.get(11)!=null){
                cStmtObject.setInt(11,new Integer((String)alSearchCriteria.get(11)));
    ...
    public Long saveMember(MemberDetailVO memberDetailVO,String strPolicyMode,String strPolicyType) throws TTKException
    {
        ...
        try{
            ...
            if(memberAddressVO.getPinCode()!=null && !memberAddressVO.getPinCode().equals("")){
                cStmtObject.setInt(PIN_CODE,new Integer(memberAddressVO.getPinCode()));
           ...
           // cStmtObject.setString(MOBILE_NO,memberAddressVO.getMobileIsdCode()+"|"+memberAddressVO.getMobileNbr());
            ...
            cStmtObject.setString(STOP_PAT_CLM_PROCESS_YN,memberDetailVO.getStopPreAuthsYN());
            cStmtObject.setTimestamp(RECEIVED_AFTER,memberDetailVO.getStopPreauthDate()!=null ? new Timestamp(memberDetailVO.getStopPreauthDate().getTime()):null);
         ...
         // END CR CHANGE
			...
			cStmtObject.setString(V_NATIONALITY_ID,memberDetailVO.getNationality());
			...
			if(memberDetailVO.getMinAge() != null){
				...
				cStmtObject.setString(v_min_age,null);
			}
		...
		//	cStmtObject.setInt(v_min_age,memberDetailVO.getMinAge());
				...
				if(memberDetailVO.getMaxAge() != null){
					...
					cStmtObject.setString(v_max_age,null);
				}
	...
	//		cStmtObject.setInt(v_max_age,memberDetailVO.getMaxAge());
		...
		//	  
			cStmtObject.setDouble(v_insurer_margin,memberDetailVO.getInsurerMargin());
			cStmtObject.setDouble(v_broker_margin,memberDetailVO.getBrokerMargin());
			cStmtObject.setDouble(v_tpa_margin,memberDetailVO.getTapMargin());
			cStmtObject.setDouble(v_reins_brk_margin,memberDetailVO.getReInsBrkMargin());
			cStmtObject.setDouble(v_other_margin,memberDetailVO.getOtherMargin());
		    cStmtObject.setDouble(v_net_prem,memberDetailVO.getNetPremium());
			// Capitation Policy  Ended 	
			System.out.println("memberDetailVO.getStopClaimsYN() :"+memberDetailVO.getStopClaimsYN());
		    //Hijri-D0B
		    cStmtObject.setTimestamp(v_hijri_dob,memberDetailVO.getHdateOfBirth()!=null ? new Timestamp(memberDetailVO.getHdateOfBirth().getTime()):null);
		    cStmtObject.setString(v_stopclaimsYN, memberDetailVO.getStopClaimsYN());
		    cStmtObject.setTimestamp(v_stopclaimsDate,memberDetailVO.getStopClaimsDate()!=null ? new Timestamp(memberDetailVO.getStopClaimsDate().getTime()):null);
            cStmtObject.setString(MAIL_NOTIFICATION_FLAG, memberDetailVO.getMailNotificationhiddenYN());
     ...
     * @exception throws TTKException
     */
    public int cancelMember(ArrayList alMember,MemberCancelVO memberCancelVO) throws TTKException
    {
        int iResult = 0;
        Connection conn = null;
        CallableStatement cStmtObject=null;
        try
        {
            ...
            cStmtObject.registerOutParameter(8, Types.INTEGER);//ROWS_PROCESSED
         	cStmtObject.setString(9,memberCancelVO.getSwitchType());
			cStmtObject.setTimestamp(10, memberCancelVO.getExitDate()!=null ? new Timestamp(memberCancelVO.getExitDate().getTime()):null);
        ...
        try{
            ...
            if (rs!=null)
            {
                while(rs.next())
                 {
                    ...
                    memberDetailVO.setMemberRemarks(TTKCommon.checkNull(rs.getString("MEM_REMARKS")));//MEMBER_REMARKS changed to REMARKS
            	...
            	/*      
            	    ...
            	    if(rs.getBigDecimal("medical_prem")!=null){
                ...
                /*    if(rs.getBigDecimal("insurer_margin")!=null){
                    	
                	      
            		memberDetailVO.setInsurerMargin(rs.setDouble("insurer_margin").toString());
                    }else{
                    	    
                    	memberDetailVO.setInsurerMargin("00.00");
                    }
                   */ 
                  ...
                  // capitation policy started
					...
					memberDetailVO.setInvoiceYN(rs.getString("invoice_gen_yn_add"));
					memberDetailVO.setCreditnoteYN(rs.getString("invoice_gen_yn_del"));
					//Ended IBM AGE CR
					
					memberDetailVO.setStopPreAuthsYN(TTKCommon.checkNull(rs.getString("stop_pat_clm_process_yn")));
					
					if(rs.getDate("RECIEVED_AFTER")!=null){
						memberDetailVO.setStopPreauthDate(rs.getDate("RECIEVED_AFTER"));
					}
					
					memberDetailVO.setStopClaimsYN(TTKCommon.checkNull(rs.getString("stop_clm_yn")));
					
					if(rs.getDate("stop_clm_date")!= null){
						memberDetailVO.setStopClaimsDate(rs.getDate("stop_clm_date"));
					}
					memberDetailVO.setMailNotificationhiddenYN(TTKCommon.checkNull(rs.getString("SEND_BDAYWISH_YN")));
...
//Added For PED
    ...
    public long saveEnrollment(MemberDetailVO memberDetailVO,String strPolicyMode,String strPolicyType) throws TTKException {
        ...
        try{
            ...
            cStmtObject.setString(30,memberDetailVO.getMemberAddressVO().getPinCode());//PIN_CODE
            cStmtObject.setString(31,memberDetailVO.getMemberAddressVO().getCountryCode());//COUNTRY_ID
           ...
           // cStmtObject.setString(36,memberDetailVO.getMemberAddressVO().getMobileIsdCode()+"|"+memberDetailVO.getMemberAddressVO().getMobileNbr());//MOBILE_NO
            ...
            cStmtObject.setString(42,memberDetailVO.getStopPreAuthsYN());
            if(memberDetailVO.getStopPreauthDate()!=null){
                cStmtObject.setTimestamp(43,new Timestamp(memberDetailVO.getStopPreauthDate().getTime()));
		...
		//	
	        ...
	        cStmtObject.setString(58, memberDetailVO.getStopClaimsYN());
	        if(memberDetailVO.getStopClaimsDate()!=null){
                cStmtObject.setTimestamp(59,new Timestamp(memberDetailVO.getStopClaimsDate().getTime()));
            }//end of if(memberDetailVO.getReceivedAfter()!=null)
            else{
                cStmtObject.setTimestamp(59,null);
            }
	        cStmtObject.setString(60,memberDetailVO.getGroupNumber());
        ...
        try{
            ...
            if(rs != null){
            	while(rs.next()){
        			...
        			memberDetailVO.setStopClaimsYN(rs.getString("stop_clm_yn"));
        			memberDetailVO.setGroupNumber(rs.getString("group_no"));
        			if(rs.getDate("stop_clm_date") !=null){
        			memberDetailVO.setStopClaimsDate(rs.getDate("stop_clm_date"));
        			}
            		if(rs.getString("DATE_OF_MARRIAGE") != null){
    ...
    public long saveBonus(SumInsuredVO sumInsuredVO,String strPolicyMode,String strPolicyType,String currency) throws TTKException{
        ...
        try {
            ...
            cStmtObject.setString(12,currency);
            cStmtObject.setLong(13,sumInsuredVO.getUpdatedBy());//ROW_PROCESSED
        ...
        try {
            ...
            if(rs != null){
            	while(rs.next()){
            		...
            		sumInsuredVO.setCurrency(TTKCommon.checkNull(rs.getString("currency_code")));
...
*/            	//else{
    ...
    //Added for the Member Medica lCertificate
 /*   public MemberDetailVO geMemberCertificate(Long enrollId) throws TTKException {
		...
		try{
			...
			conn = ResourceManager.getConnection();	
				cStmtObject = (java.sql.CallableStatement) conn.prepareCall(strGetMemberMedicalCertificate);
				cStmtObject.setLong(1,enrollId);//EnrollId
				cStmtObject.registerOutParameter(2,OracleTypes.CURSOR);
				cStmtObject.execute();
				rs = (java.sql.ResultSet)cStmtObject.getObject(2);
			if(rs != null)
			{
				if(rs.next())
				{
					memberDetailVO.setPolicyNbr(TTKCommon.checkNull(rs.getString("policy_number")));

					memberDetailVO.setPolicyStartDate(new Date(rs.getTimestamp("effective_from").getTime()));	
					
					memberDetailVO.setEndDate(new Date(rs.getTimestamp("effective_to").getTime()));					 

					memberDetailVO.setStartDate(new Date(rs.getTimestamp("effective_from").getTime()));				

					memberDetailVO.setName(TTKCommon.checkNull(rs.getString("member_name")));					
					
					memberDetailVO.setBeneficiaryname(TTKCommon.checkNull(rs.getString("member_name")));	
					
					memberDetailVO.setGroupName(TTKCommon.checkNull(rs.getString("group_name")));					

					memberDetailVO.setEnrollmentID(TTKCommon.checkNull(rs.getString("alkoot_id")));				

					memberDetailVO.setDateOfBirth(new Date(rs.getTimestamp("dob").getTime()));					

					memberDetailVO.setEnrollmentID(TTKCommon.checkNull(rs.getString("alkoot_id")));	
					
					memberDetailVO.setGenderDescription(TTKCommon.checkNull(rs.getString("gender")));
					
					memberDetailVO.setGenderTypeID(TTKCommon.checkNull(rs.getString("gender")));
					
					memberDetailVO.setPassportNumber(TTKCommon.checkNull(rs.getString("passport_number")));					

					memberDetailVO.setNationality(TTKCommon.checkNull(rs.getString("nationality")));	
		...
		finally
		{
	    	...
	    	try // First try closing the Statement
	    	{
	    		...
	    		{
	    			log.error("Error while closing the Statement in MemberDAOImpl geMemberCertificate()",sqlExp);
	    			throw new TTKException(sqlExp, "member");
	    		}//end of catch (SQLException sqlExp)
	    		finally // Even if statement is not closed, control reaches here. Try closing the connection now.
	    		{
	    			...
	    			{
	    				log.error("Error while closing the Connection in MemberDAOImpl geMemberCertificate()",sqlExp);
	    				throw new TTKException(sqlExp, "member");
	    			}//end of catch (SQLException sqlExp)
	    			finally{
	    				try{
		    				...
		    				log.error("Error while closing the ResultSet in MemberDAOImpl geMemberCertificate()",sqlExp);
	    				}
	    			}
	    		}//end of finally Connection Close
	    	}//end of try
		...
		return memberDetailVO;
	}//end of geMemberCertificate(String EnrollId	
    */
    
    public RuleVO getMemPolicyRule(Long memSeqId) throws TTKException {
		Document doc = null;
        XMLType xml = null;
        Connection conn = null;
        ...
        try{
        	conn = ResourceManager.getConnection();
			stmt = (OracleCallableStatement)((OracleConnection)((WrappedConnectionJDK6)conn).getUnderlyingConnection()).prepareCall(strGetMemPolicyRule);
			stmt.setLong(1,memSeqId);
			...
			if(rs != null){
				while(rs.next()){
					...
					if(rs.getString("member_seq_id") != null){
					ruleVO.setMemberSeqID(new Long(rs.getLong("member_seq_id")));
					}
					}//end of while(rs.next())
			}//end of if(rs != null)
			return ruleVO;
		...
		finally
		{
			...
			try // First try closing the result set
			{
				...
				{
					log.error("Error while closing the Resultset in MemberDAOImpl getMemPolicyRule()",sqlExp);
					throw new TTKException(sqlExp, "member");
				}//end of catch (SQLException sqlExp)
				finally // Even if result set is not closed, control reaches here. Try closing the statement now.
				{
					...
					{
						log.error("Error while closing the Statement in MemberDAOImpl getMemPolicyRule()",sqlExp);
						throw new TTKException(sqlExp, "member");
					}//end of catch (SQLException sqlExp)
					finally // Even if statement is not closed, control reaches here. Try closing the connection now.
					{
						...
						{
							log.error("Error while closing the Connection in MemberDAOImpl getMemPolicyRule()",sqlExp);
							throw new TTKException(sqlExp, "member");
						}//end of catch (SQLException sqlExp)
					}//end of finally Connection Close
				}//end of finally Statement Close
    ...
    //CR 0616
	public int uploadMemberMdfFiles(ArrayList<Object> alAddMdfParamsList) throws TTKException {
        int iResult = 0;
        Connection conn = null;
        CallableStatement cStmtObject=null;
        try{
        	conn = ResourceManager.getConnection();
            cStmtObject = (java.sql.CallableStatement)conn.prepareCall(strSaveMemberMdf);       
            cStmtObject.setString(1,(String) alAddMdfParamsList.get(0));
            cStmtObject.setString(2,(String) alAddMdfParamsList.get(1));
            cStmtObject.setString(3,(String) alAddMdfParamsList.get(2));
            cStmtObject.setString(4,(String) alAddMdfParamsList.get(3));
			cStmtObject.setLong(5,(long) alAddMdfParamsList.get(4));
        ...
        finally
		{
        	...
        	try // First try closing the Statement
        	{
        		...
        		{
        			log.error("Error while closing the Statement in MemberDAOImpl uploadMemberMdfFiles()",sqlExp);
        			throw new TTKException(sqlExp, "members");
        		}//end of catch (SQLException sqlExp)
        		finally // Even if statement is not closed, control reaches here. Try closing the connection now.
        		{
        			...
        			{
        				log.error("Error while closing the Connection in MemberDAOImpl uploadMemberMdfFiles()",sqlExp);
        				throw new TTKException(sqlExp, "members");
        			}//end of catch (SQLException sqlExp)
        		}//end of finally Connection Close
        	}//end of try
	...
	//CR 0616
	public ArrayList<MemberVO> getUploadedMdfFiles(MemberVO memberVO) throws TTKException {
    	
		ArrayList<MemberVO> alUploadedMdfFiles = new ArrayList<MemberVO>();
        Connection conn = null;
        CallableStatement cStmtObject=null;
        ResultSet rs = null;
        String memList[]	=	null;
        ...
        try {
            ...
            conn = ResourceManager.getConnection();
            cStmtObject = (java.sql.CallableStatement)conn.prepareCall(strUploadedMdfFilesList);
            if(memberVO.getMemberSeqID()==null)
            	cStmtObject.setString(1,null);
            else
            	cStmtObject.setLong(1,memberVO.getMemberSeqID());
            if(memberVO.getPolicyGroupSeqID()==null)
            	cStmtObject.setString(2,null);
            else
            	cStmtObject.setLong(2,memberVO.getPolicyGroupSeqID());
            cStmtObject.setString(3,"ENR");
            cStmtObject.registerOutParameter(4,OracleTypes.CURSOR);
            cStmtObject.execute();
            rs = (java.sql.ResultSet)cStmtObject.getObject(4);
            System.out.println("memseqId::"+memberVO.getMemberSeqID()+" and polisy::"+memberVO.getPolicyGroupSeqID());
            
            if (rs!=null)
            {
            	while(rs.next())
                {
            		/*memList	=	new String[4];
            		memList[0]	=	rs.getString("TPA_ENROLLMENT_ID");
            		memList[1]	=	rs.getString("MEM_NAME");
            		memList[2]	=	rs.getString("FILE_NAME");
            		memList[3]	=	rs.getString("DATE_OF_UPLOAD");*/
            		
            		vos	=	new MemberVO();
            		vos.setEnrollmentID(rs.getString("TPA_ENROLLMENT_ID"));
            		vos.setMemName(rs.getString("MEM_NAME"));
            		vos.setName(rs.getString("FILE_NAME"));
            		vos.setUpdatedDate(rs.getDate("DATE_OF_UPLOAD"));
            		vos.setMemberSeqID(rs.getLong("MEMBER_SEQ_ID"));
            		vos.setStrFamilyName(rs.getString("FILE_NAME1_TIMESTAMP"));
            		vos.setMemberTypeID(rs.getString("MEM_MDF_SEQ_ID"));
                    
            		alUploadedMdfFiles.add(vos);
                }// End of while (rs.next())
            }// End of if (rs!=null)
        }// end of try
        catch (SQLException sqlExp)
        ...
        return (ArrayList) alUploadedMdfFiles;
    }

	public int deleteMemberMdfFiles(String memSeqId, String mdfSeqId, Long userSeqId) throws TTKException{
        int iResult = 0;
        Connection conn = null;
        CallableStatement cStmtObject=null;
        try{
        	conn = ResourceManager.getConnection();
            cStmtObject = (java.sql.CallableStatement)conn.prepareCall(strDeleteMemberMdf);
            cStmtObject.setString(1, memSeqId);
            cStmtObject.setString(2, mdfSeqId);
			cStmtObject.setLong(3,userSeqId);//ADDED_BY
        ...
        finally
		{
        	...
        	try // First try closing the Statement
        	{
        		...
        		{
        			log.error("Error while closing the Statement in MemberDAOImpl deleteMemberMdfFiles()",sqlExp);
        			throw new TTKException(sqlExp, "member");
        		}//end of catch (SQLException sqlExp)
        		finally // Even if statement is not closed, control reaches here. Try closing the connection now.
        		{
        			...
        			{
        				log.error("Error while closing the Connection in MemberDAOImpl deleteMemberMdfFiles()",sqlExp);
        				throw new TTKException(sqlExp, "member");
        			}//end of catch (SQLException sqlExp)
        		}//end of finally Connection Close
        	}//end of try
