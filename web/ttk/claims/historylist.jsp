...
<html:form action="/ClaimHistoryAction.do">
	<html:errors/>
	...
	<table align="center" class="searchContainer" border="0" cellspacing="0" cellpadding="0">
		<tr class="searchContainerWithTab">
		    <td nowrap>History Mode:<br>
          	...
          	<td nowrap>Sort By:<br>
	            <html:select property="sortBy" styleClass="selectBox selectBoxMedium" name="frmHistoryList" onchange="changeHistoryMode()">
    	...
    	<td>
    	 ...
    	 <td nowrap>Ascending  <html:radio property="ascOrDesc" value="ASC" styleId="asc" onclick="changeBenefitTypeMode()"/>
	...
	<table align="center" class="formContainer" border="1" cellspacing="0" cellpadding="0"  frame="box" rules="all">
			<tr  class="gridHeader">
			<th align="center" style="width:10%">PreApproval No.</th>
  ...
  <logic:notEmpty name="claimHistoryList" scope="session">  
    <% 
     historylistcode = (ArrayList<String[]>)session.getAttribute("claimHistoryList");
    int i=0;
    String[] historylistcodestringarray = new String[16];
  %>
      <c:forEach items="${sessionScope.claimHistoryList}" var="historyList">       
        <%
       ...
       <tr>
		...
		<% 
				...
				if("Y".equals(EnhancedYN[15])){%>
				 <img alt="Enhanced" src="/ttk/images/EnhancedPreauth.gif" style="size: 10px" width="18%" height="15%"> 
			 <%}} %>
		</td>
...
<td align="right" colspan="475"> 
 <button type="submit"  name="Button1" accesskey="c" class="buttons" onClick="javascript:generateDownloadPATCLM();" onMouseout="this.className='buttons'" onMouseover="this.className='buttons buttonsHover'"><u>D</u>ownload to Excel</button>
 &nbsp;&nbsp;
...
<logic:equal value="CLM" name="frmHistoryList" property="historyMode">
	...
	<table align="center" class="formContainer" border="1" cellspacing="0" frame="box" rules="all">
			...
			<th align="center" style="width:2%;" >Invoice No.</th>
			<th align="center" style="width:13%;">Pre Approval No. Linked</th>
			<th align="center" style="width:9%;">Benefit Type</th>
      ...
      <c:forEach items="${sessionScope.claimHistoryList}" var="historyList">  
       ...
       <td align="center"><c:out value="${historyList[9]}"/></td>
		...
		if("N".equals(fraudYN[11]) && "Y".equals(fraudYN[10])){
			...
			<c:out value="${historyList[1]}"/><img alt="Alkoot Clearification" src="/ttk/images/suspectedcheckedfraud.gif" width="13" height="13">	
	    <% 	}else if(fraudYN[12].equals("PROV")){%>
	    	<c:out value="${historyList[1]}"/><img alt="Provider Clearification" src="/ttk/images/suspectedcheckedfraud.gif" width="13" height="13">
	   <% }else if(fraudYN[12].equals("SUSP")){%>
		   <c:out value="${historyList[1]}"/><img alt="Suspected Claim" src="/ttk/images/suspectedcheckedfraud.gif" width="13" height="13">
	 <%  }
		...
		<%} 
			...
			if(fraudYN[12].equals("ALKT")){%>
				<c:out value="${historyList[1]}"/><img alt="Alkoot Clearification-Verified" src="/ttk/images/verifiedclaimsearchAndhistoryimg.gif" width="13" height="13">
		<%	}else if(fraudYN[12].equals("PROV")){%>
		<c:out value="${historyList[1]}"/><img alt="Provider Clearification-Verified" src="/ttk/images/verifiedclaimsearchAndhistoryimg.gif" width="13" height="13">	
	<%	}else if(fraudYN[12].equals("SUSP")){%>
				<c:out value="${historyList[1]}"/><img alt="Suspected Claim-Verified" src="/ttk/images/verifiedclaimsearchAndhistoryimg.gif" width="13" height="13">	
...
<td><b>Total Amount:<bean:write name="frmHistoryList" property="sum"/></b></td>
<td align="right" colspan="475"> 
 <button type="submit"  name="Button1" onClick="javascript:generateDownloadPATCLM();" accesskey="c" class="buttons" onMouseout="this.className='buttons'" onMouseover="this.className='buttons buttonsHover'"><u>D</u>ownload to Excel</button>
&nbsp;&nbsp;
