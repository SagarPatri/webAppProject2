...
public class ClaimBillDAOImpl implements BaseDAO, Serializable{
	...
	private static final String strGetBillSummaryDetail = "{CALL CLAIMS_PKG.SELECT_BILL_SUMMARY(?,?,?,?,?,?)}";
	private static final String strCheckClaim = "{CALL CLAIMS_PKG.CHECK_CLAIM(?,?)}";
	...
	private static final String strGetBillList = "{CALL CLAIMS_PKG.CLAIMS_BILL_HEADER_LIST(?,?,?,?,?,?,?,?,?,?)}";
	private static final String strIncludeOrExcludeBillInfo = "{CALL CLAIMS_PKG.EXCLUDE_BILLS(?,?,?,?,?,?)}";
	//private static final String strGetBillDetail = "{CALL CLAIMS_SQL_PKG.SELECT_BILL_HEADER(?,?,?)}";
	private static final String strGetBillDetail = "{CALL CLAIMS_PKG.SELECT_BILL_HEADER(?,?,?)}";
	private static final String strSaveBillDetail = "{CALL CLAIMS_PKG.SAVE_CLM_BILL_HEADER(?,?,?,?,?,?,?,?,?,?)}";//added for donor expenses
	//private static final String strGetLineItemList = "{CALL CLAIMS_SQL_PKG.CLAIMS_BILL_DETAILS_LIST(?,?,?,?,?,?,?)}";
	private static final String strGetLineItemList = "{CALL CLAIMS_PKG.CLAIMS_BILL_DETAILS_LIST(?,?,?,?,?,?,?)}";
    //added for KOC-1356 we removed _DATA_ENTRY
	private static final String strSaveLineItemDetailNext = "{CALL CLAIMS_PKG.select_bills_save_next(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
    private static final String strSaveLineItemDetail = "{CALL CLAIMS_PKG.SAVE_CLM_BILL_DETAILS(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";//added for maternity//changes for koc 1315 1316 1308
	private static final String strGetBillLineItemList = "{CALL CLAIMS_PKG.SELECT_BILLS_LIST(?,?,?)}";
	//add ended
	private static final String strGetLineitemList = "{CALL CLAIMS_PKG.SELECT_LINEITEMS_LIST(?,?)}";
	private static final String strGetLineitem = "{CALL CLAIMS_PKG.SELECT_LINEITEM(?,?)}";
	...
	private static final String strAccountType = "select min(gen.claim_sub_general_type_id) claim_sub_general_type_id  from clm_general_details gen where gen.claim_seq_id=?";
		...
		try{
			...
			if(rs != null){
				while(rs.next()){
					...
					claimBillDetailVO.setClaimSubTypeID(TTKCommon.checkNull(rs.getString("CLAIM_SUB_GENERAL_TYPE_ID")));
					//physiotherapy cr 1320
		...
		try{
			...
			if(rs != null){
				while(rs.next()){
					...
					lineItemVO.setImmuneTypeID(TTKCommon.checkNull(rs.getString("immunization_type_id")));//added for koc 1315
					lineItemVO.setWellchildTypeID(TTKCommon.checkNull(rs.getString("well_child_test_type_id")));//added for koc 1316
					lineItemVO.setRoutadultTypeID(TTKCommon.checkNull(rs.getString("rout_adult_vacc_type_id")));//added for koc 1308
					...
					if(rs.getString("NO_OF_VISITS") != null){
						lineItemVO.setNumberofVisits(new Integer(rs.getInt("NO_OF_VISITS")));
					}//end of if(rs2.getString("NUMBER_OF_DAYS") != null)
					else
					{
						lineItemVO.setNumberofVisits(0);
					}
					lineItemVO.setClaimSubTypeID(TTKCommon.checkNull(rs.getString("CLAIM_SUB_GENERAL_TYPE_ID")));
					//physiotherapy cr 1320
	...
	public long saveLineItemDetail(LineItemVO lineItemVO) throws TTKException {
		...
		try{
            ...
            cStmtObject.setString(13,lineItemVO.getImmuneTypeID());//added for koc 1315
			cStmtObject.setString(14,lineItemVO.getWellchildTypeID());//added for koc 1316
            cStmtObject.setString(15,lineItemVO.getRoutadultTypeID());//added for koc 1308
            cStmtObject.setInt(16, lineItemVO.getNumberofVisits()!=null ? lineItemVO.getNumberofVisits():0);
		...
		try{
			...
			if(rs1 != null){
				while(rs1.next()){
					...
					//physiotherapy cr 1320
					claimBillDetailVO.setClaimSubTypeID(TTKCommon.checkNull(rs1.getString("CLAIM_SUB_GENERAL_TYPE_ID")));
					//physiotherapy cr 1320
					...
					if(rs2 != null){
						while (rs2.next()){
							...
							lineItemVO.setImmuneTypeID(TTKCommon.checkNull(rs2.getString("immunization_type_id")));//added for koc 1315
							lineItemVO.setWellchildTypeID(TTKCommon.checkNull(rs2.getString("well_child_test_type_id")));//added for koc 1316
							lineItemVO.setRoutadultTypeID(TTKCommon.checkNull(rs2.getString("rout_adult_vacc_type_id")));//added for koc 1308
							...
							//physiotherapy cr 1320
							if(rs2.getString("NO_OF_VISITS") != null){
								lineItemVO.setNumberofVisits(new Integer(rs2.getInt("NO_OF_VISITS")));
							}//end of if(rs2.getString("NUMBER_OF_DAYS") != null)
							else
							{
								lineItemVO.setNumberofVisits(0);
							}
							lineItemVO.setClaimSubTypeID(TTKCommon.checkNull(rs2.getString("CLAIM_SUB_GENERAL_TYPE_ID")));
							//physiotherapy cr 1320
	...
	public String getAccountType(long lngClaimSeqID)throws TTKException
	{
		...
		{
			conn = ResourceManager.getConnection();
			pStmt = conn.prepareStatement(strAccountType);
			pStmt.setLong(1,lngClaimSeqID);
			...
			if(rs != null){
				...
				while (rs.next()) {
					...
					AccountType = TTKCommon.checkNull(rs.getString("claim_sub_general_type_id"));					
				}
			}
		...
		finally
		{
			...
			try // First try closing the result set
			{
				...
				{
					log.error("Error while closing the Resultset in ClaimBillDAOImpl getAccountType()",sqlExp);
					throw new TTKException(sqlExp, "bill");
				}//end of catch (SQLException sqlExp)
				finally // Even if result set is not closed, control reaches here. Try closing the statement now.
				{
					...
					{
						log.error("Error while closing the Statement in ClaimBillDAOImpl getAccountType()",sqlExp);
						throw new TTKException(sqlExp, "bill");
					}//end of catch (SQLException sqlExp)
					finally // Even if statement is not closed, control reaches here. Try closing the connection now.
					{
