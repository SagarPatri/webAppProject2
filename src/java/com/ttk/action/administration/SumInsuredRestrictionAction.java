...
/**
 * @ (#) SumInsuredRestrictionAction.java Jun 23, 2011
...
import com.ttk.dto.administration.SumInsuredRestrictionVO;

import formdef.plugin.util.FormUtils;


public class SumInsuredRestrictionAction extends TTKAction  {
	
	//private static final Logger log = Logger.getLogger( CopayConfigurationAction.class );
	
	private static final String strProductConfigSumInsured="prodconfigsuminsured";
	private static final String strPolicyConfigSumInsured="policyconfigsuminsured";
	...
	public ActionForward  doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
			HttpServletResponse response) throws TTKException 
			{
				try
				{
					...
					DynaActionForm frmConfigSumInsured = (DynaActionForm)form;
					...
					if("Products".equals(strActiveSubLink)){
						strForwardPath=strProductConfigSumInsured;
					}//end of if("Products".equals(strActiveSubLink))
					else {
						strForwardPath=strPolicyConfigSumInsured;
					}//end of else
					Long  lngProdPolicySeqId = TTKCommon.getWebBoardId(request);
					sumInsuredRestrictionVO = productpolicyObject.getConfigSumInsuredRestrictedAmt(lngProdPolicySeqId);
					frmConfigSumInsured = (DynaActionForm)FormUtils.setFormValues("frmConfigSumInsured",sumInsuredRestrictionVO,this,mapping,request);
					frmConfigSumInsured.set("caption",buildCaption(request));
					request.getSession().setAttribute("frmConfigSumInsured",frmConfigSumInsured);
			...
			HttpServletResponse response) throws TTKException
	{
		try
		{
			...
			String strForwardPath=null;
			DynaActionForm frmConfigSumInsured = (DynaActionForm)form;
			SumInsuredRestrictionVO sumInsuredRestrictionVO=new SumInsuredRestrictionVO();
			frmConfigSumInsured.set("caption",buildCaption(request));
			sumInsuredRestrictionVO =(SumInsuredRestrictionVO)FormUtils.getFormValues(frmConfigSumInsured, "frmConfigSumInsured",this, mapping, request);
		
			
			if(frmConfigSumInsured.getString("suminsuredrestrictionYN").equalsIgnoreCase("N"))
			{
				sumInsuredRestrictionVO=null;
				sumInsuredRestrictionVO=new SumInsuredRestrictionVO();
				sumInsuredRestrictionVO.setSuminsuredrestrictionYN("N");
				sumInsuredRestrictionVO.setFixedAmtRestriction(null);
				sumInsuredRestrictionVO.setMembersrelation(null);
				sumInsuredRestrictionVO.setPercentageRestriction(null);
				sumInsuredRestrictionVO.setAgeRestricted(null);
			...
			sumInsuredRestrictionVO.setLngProdPolicySeqID(TTKCommon.getWebBoardId(request));
			sumInsuredRestrictionVO.setUpdatedBy(TTKCommon.getUserSeqId(request));
			
		
			ArrayList allOutParam = productpolicyObject.saveConfigSumInsuredRestrictedAmt(sumInsuredRestrictionVO,"FPR");
			...
			if(iResult>0)
			{
					...
					sumInsuredRestrictionVO = productpolicyObject.getConfigSumInsuredRestrictedAmt(TTKCommon.getWebBoardId(request));
					frmConfigSumInsured = (DynaActionForm)FormUtils.setFormValues("frmConfigSumInsured",sumInsuredRestrictionVO, this, mapping, request);
			}//end of if(iResult>0)
			frmConfigSumInsured.set("caption",buildCaption(request));
