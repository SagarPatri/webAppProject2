...
public class HistoryAction extends TTKAction  {
	...
	public ActionForward doDefault(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			 ...
			 if(strLink.equals("Support"))
			{
				...
				if(request.getSession().getAttribute("invInvestigationVO") == null)
				{
				...
				InvestigationVO invInvestigationVO=(InvestigationVO)request.getSession().getAttribute("invInvestigationVO");				
				strClaimantName = TTKCommon.checkNull(invInvestigationVO.getClaimantName());				
				if("".equals(invInvestigationVO.getEnrollmentID().trim())&& invInvestigationVO.getEnrollmentID()== null)
				{
					TTKException expTTK = new TTKException();
					expTTK.setMessage("error.investigation.required");
					throw expTTK;
				}//end of if(strClaimantName)				
				if(!"".equals(invInvestigationVO.getEnrollmentID().trim())&&
						invInvestigationVO.getEnrollmentID()!= null)
				{
					strEnrollmentID= strEnrollmentID+" [ "+invInvestigationVO.getEnrollmentID()+" ]";
			...
			else if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HIN"))
			{
				strTableName = "InvestigationHistory";
			}//end of else if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HIN"))
	...
	public ActionForward doSearch(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		try
		{
			...
			else if(strLink.equals("Support"))
			{
				strHistorylist = "claimhistorylist";
				InvestigationVO invInvestigationVO=(InvestigationVO)request.getSession().getAttribute("invInvestigationVO");
				strClaimantName = invInvestigationVO.getClaimantName();//ClaimsWebBoardHelper.getClaimantName(request);
	...
	public ActionForward doView2(ActionMapping mapping,ActionForm form,HttpServletRequest request,
		...
		{
			setLinks(request);
			log.debug("Inside HistoryAction doView2");
			MemberHistoryManager memberHistoryManagerObject=this.getMemberHistoryManagerObject();
			...
			long hseqId=0;
			try{
				hseqId=Long.parseLong(seqId);
			}catch(Exception e){}
		   historyDoc=memberHistoryManagerObject.getPolicyHistory(hseqId);//request.getParameter("lngSeqId"));
			request.setAttribute("historyDoc",historyDoc);
			return mapping.findForward("preauthViewHistory");
		}//end of try
		catch(TTKException expTTK)
		{
			return this.processExceptions(request, mapping, expTTK);
...
InvestigationHistoryVO investigationHistoryVO= null;
...
else if(strLink.equals("Support")) //koc11 koc 11
{
strFwdHistory = "historydetails";// InvestigationHistoryDetails
InvestigationVO invInvestigationVO=(InvestigationVO)request.getSession().getAttribute("invInvestigationVO");
strClaimantName = invInvestigationVO.getClaimantName();//ClaimsWebBoardHelper.getClaimantName(request);
...
else if(((String)frmHistoryList.get("PreAuthHistoryTypeID")).equals("HIN"))
{
...
strPreAuthHistoryTypeID=(String)frmHistoryList.get("PreAuthHistoryTypeID");
investigationHistoryVO=(InvestigationHistoryVO)tableData.getRowInfo(Integer.parseInt(
request.getParameter("rownum")));
strLinkMode = (String)request.getSession().getAttribute("LinkMode");					
lngInvestSeqID = investigationHistoryVO.getInvestSeqID();
lngPreAuthSeqID = investigationHistoryVO.getPreAuthSeqID();
lngClaimsSeqID = investigationHistoryVO.getClaimSeqID();
	...
	private ArrayList<Object> populateSearchCriteria(DynaActionForm frmHistory,HttpServletRequest request)
	{
		...
		try{
			...
			if(strLink.equals("Pre-Authorization"))
			{
				...
				{
					alSearchParams.add("HIN"); //koc11 koc 11
				}//end of if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HIN"))
			}//end of if(strLink.equals("Pre-Authorization"))
			else if(strLink.equals("Claims"))
			{
			...
			{				
				InvestigationVO invInvestigationVO=(InvestigationVO)request.getSession().getAttribute("invInvestigationVO");
				strEnrollmentID = invInvestigationVO.getEnrollmentID();						
				
				if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HPR"))
				{
					alSearchParams.add("HPR");
					lngSeqId = invInvestigationVO.getEnrollDtlSeqID();   //PAT_ENROLL_DETAIL_SEQ_ID
				}//end of if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HPR"))				
				else if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HMP"))  
				{
					alSearchParams.add("HMP");
					lngSeqId = invInvestigationVO.getEnrollDtlSeqID();   //PAT_ENROLL_DETAIL_SEQ_ID //enrollmentid not enrollment sequence id
				}//end of else if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("HMP"))
				else if(((String)frmHistory.get("PreAuthHistoryTypeID")).equals("POL"))
				{
					alSearchParams.add("POL");
