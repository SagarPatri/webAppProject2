/**
 * @ (#) CodingReportsListAction.java June 19, 2008
 * Project      : TTK HealthCare Services
 * File         : CodingReportsListAction.java
...
public class CodingReportsListAction extends TTKAction
{
private static final Logger log = Logger.getLogger( CodingReportsListAction.class );
	
	//string for forwarding
    private static final String strCodeReportList="codereportlist";
    private static final String strCodeReportparam="codereportparam";
    ...
    {
    	try{
    		log.debug("Inside the doDefault method of CodingReportsListAction");
    		setLinks(request);
    		if (TTKCommon.checkNull(request.getParameter("Entry")).equals("Y"))
    		{
    			((DynaActionForm) form).initialize(mapping);// reset the form data
    		}//end of if (TTKCommon.checkNull(request.getParameter("Entry")).equals("Y"))
    		return this.getForward(strCodeReportList, mapping, request);
		}// end of try
    ...
    /**
	  ...
	  * @throws Exception if any error occurs
	  */
  public ActionForward doVeiw(ActionMapping mapping,ActionForm form,HttpServletRequest request,
          							  HttpServletResponse response) throws TTKException
  {
  	try{
  		log.debug("Inside the doVeiw method of CodingReportsListAction");
  		setLinks(request);
  		
  		return this.getForward(strCodeReportparam, mapping, request);
		}//end of try
  		catch(TTKException expTTK)
		{
			return this.processExceptions(request, mapping, expTTK);
   ...
   {
	   try{
		   log.debug("Inside the doGenerateReport method of CodingReportsListAction");
		   setLinks(request);
...
}//end of CodingReportsListAction class
